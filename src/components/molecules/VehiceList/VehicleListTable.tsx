      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gray-50 hover:bg-gray-50">
            <TableHead className="lg:max-w-auto max-w-[90px] truncate">Matrícula o VIN</TableHead>
        ...
        <TableBody>
          {vehicles.map((vehicle) => (
            <TableRow key={vehicle._id} className="transition-colors hover:bg-gray-50">
              <TableCell className="lg:max-w-auto max-w-[90px] truncate">
                {vehicle.plate || vehicle.vinCode || '—'}
              </TableCell>
              <TableCell>{vehicle.brand || '—'}</TableCell>
              <TableCell className="lg:max-w-auto max-w-[35px] truncate">
                {vehicle.model || '—'}
              </TableCell>
              ...
              <TableCell className="text-right">
                <Link to={`/cars/${vehicle._id}`}>
                  <Button variant="link" size="sm">
                    Ver <span className="hidden lg:table-cell">Detalles</span>
                  </Button>
                </Link>
              </TableCell>
            </TableRow>
