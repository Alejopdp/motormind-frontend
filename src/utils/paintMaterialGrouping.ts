import {
  PaintMaterialType,
  PAINT_MATERIAL_LABELS,
  PAINT_MATERIAL_CODES,
} from '@/types/PaintMaterial';
...
export const groupPaintMaterials = (damages: Damage[]): GroupedPaintMaterial[] => {
  ...
  console.log(
    'Damages with paintWorks:',
    damages.filter((d) => d.paintWorks && d.paintWorks.length > 0).length,
  );
  ...
  // Agrupar por tipo
  const groupedByType = allPaintWorks.reduce(
    (groups, paintWork) => {
  ...
  const groupedMaterials: GroupedPaintMaterial[] = Object.entries(groupedByType).map(
    ([type, items]) => {
 ...
 */
export const calculatePaintMaterialsSubtotal = (
  groupedMaterials: GroupedPaintMaterial[],
): number => {
